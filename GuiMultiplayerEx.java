...
public class GuiMultiplayerEx extends GuiScreen {
	...
	private GuiSlotServerEx serverSlotContainer;
	...
	public GuiMultiplayerEx(GuiScreen par1GuiScreen) {
	...
	public void initGui() {
		...
		if (!this.field_74024_A) {
			...
			this.serverSlotContainer = new GuiSlotServerEx(this);
		} else {
	...
	public void initGuiControls() {
		...
		this.buttonList.add(this.field_96289_p = new GuiButton(7, this.width / 2 - 154, this.height - 28, 70, 20, stringtranslate
				.translateKey("selectServer.edit")));
		this.buttonList.add(this.buttonDelete = new GuiButton(2, this.width / 2 - 74, this.height - 28, 70, 20, stringtranslate
				.translateKey("selectServer.delete")));
		this.buttonList.add(this.buttonSelect = new GuiButton(1, this.width / 2 - 154, this.height - 52, 100, 20, stringtranslate
				.translateKey("selectServer.select")));
		this.buttonList.add(new GuiButton(4, this.width / 2 - 50, this.height - 52, 100, 20, stringtranslate
				.translateKey("selectServer.direct")));
		this.buttonList.add(new GuiButton(3, this.width / 2 + 4 + 50, this.height - 52, 100, 20, stringtranslate
				.translateKey("selectServer.add")));
		this.buttonList.add(new GuiButton(8, this.width / 2 + 4, this.height - 28, 70, 20, stringtranslate
				.translateKey("selectServer.refresh")));
		this.buttonList
				.add(new GuiButton(0, this.width / 2 + 4 + 76, this.height - 28, 75, 20, stringtranslate.translateKey("gui.cancel")));
	...
	protected void actionPerformed(GuiButton par1GuiButton) {
		if (par1GuiButton.enabled) {
			if (par1GuiButton.id == 2) {
				...
				this.mc.displayGuiScreen(new GuiScreenServerList(this, this.theServerData = new ServerData(StatCollector
						.translateToLocal("selectServer.defaultName"), "")));
			} else if (par1GuiButton.id == 3) {
				this.addClicked = true;
				this.mc.displayGuiScreen(new GuiScreenAddServer(this, this.theServerData = new ServerData(StatCollector
						.translateToLocal("selectServer.defaultName"), "")));
				...
				this.mc.displayGuiScreen(new GuiMultiplayerEx(this.parentScreen));
			} else {
	...
	private void connectToServer(ServerData par1ServerData) {
		//TODO: Changed (not a todo, just want the blue mark)
		AutoRelog.serverData = par1ServerData;
		...
		try {
			...
			if (s.startsWith("\u00a7") && s.length() > 1) {
				...
				if (MathHelper.parseIntWithDefault(astring[0], 0) == 1) {
					...
					if (j >= 0 && k >= 0) {
						par1ServerData.populationInfo = EnumChatFormatting.GRAY + "" + j + "" + EnumChatFormatting.DARK_GRAY + "/"
	...
	static ServerList getInternetServerList(GuiMultiplayerEx par0GuiMultiplayer) {
		return par0GuiMultiplayer.internetServerList;
	}

	static List getListOfLanServers(GuiMultiplayerEx par0GuiMultiplayer) {
		return par0GuiMultiplayer.listofLanServers;
	}

	static int getSelectedServer(GuiMultiplayerEx par0GuiMultiplayer) {
		return par0GuiMultiplayer.selectedServer;
	}

	static int getAndSetSelectedServer(GuiMultiplayerEx par0GuiMultiplayer, int par1) {
	 ...
	 */
	static GuiButton getButtonSelect(GuiMultiplayerEx par0GuiMultiplayer) {
	 ...
	 */
	static GuiButton getButtonEdit(GuiMultiplayerEx par0GuiMultiplayer) {
	 ...
	 */
	static GuiButton getButtonDelete(GuiMultiplayerEx par0GuiMultiplayer) {
		return par0GuiMultiplayer.buttonDelete;
	}

	static void func_74008_b(GuiMultiplayerEx par0GuiMultiplayer, int par1) {
		par0GuiMultiplayer.joinServer(par1);
	}

	static int getTicksOpened(GuiMultiplayerEx par0GuiMultiplayer) {
	...
	static String getAndSetLagTooltip(GuiMultiplayerEx par0GuiMultiplayer, String par1Str) {
